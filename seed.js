...
async function main() {
    ...
    let teddy=await tools.addTool("Teddy","very fluffy teady bear", "Ok", neel._id.toString(), ["on",undefined,"on",undefined,"on",undefined,"on"], "Hoboken, NJ, USA","1724515479001.png")
    teddy=teddy.insertedId.toString()
    let firetruck=await tools.addTool("Firetruck","big red firetruck", "Like New", tara._id.toString(), ["on",undefined,undefined,undefined,"on",undefined,undefined], "Hoboken, NJ, USA","1724515479002.jpeg")
    firetruck=firetruck.insertedId.toString()
    let ambulance=await tools.addTool("Ambulance","super safe ambulance", "Very Damaged", kashish._id.toString(), [undefined,undefined,"on",undefined,undefined,undefined,undefined], "Hoboken, NJ, USA","1724515479003.jpeg")
    ambulance=ambulance.insertedId.toString()
    let police=await tools.addTool("Police car","fast and furious ride", "Good", patrick._id.toString(), [undefined,undefined,"on",undefined,"on",undefined,undefined], "Hoboken, NJ, USA","1724515479004.jpeg")
    police=police.insertedId.toString()
    let paint=await tools.addTool("Paint brushes","long paint brushes (paint not included)", "Very Good", jey._id.toString(), ["on","on","on","on","on","on","on"], "Hoboken, NJ, USA","1724515479005.jpeg")
    paint=paint.insertedId.toString()
    let fell=await tools.addTool("Computer","Fell HumanWare Computer (might contain virus)", "Very Damaged", adib._id.toString(), ["on","on","on",undefined,"on","on",undefined], "Hoboken, NJ, USA","1724515479006.jpeg")
    fell=fell.insertedId.toString()
    let esus=await tools.addTool("Computer","ESUS MenBook (antivirus firewall included)", "Like New", neel._id.toString(), ["on","on","on",undefined,"on",undefined,undefined], "Hoboken, NJ, USA","1724515479007.jpeg")
    esus=esus.insertedId.toString()
    let missiles=await tools.addTool("Missiles","fast and explosive", "Minor Damage", tara._id.toString(), ["on","on","on",undefined,undefined,undefined,undefined], "Hoboken, NJ, USA","1724515479008.jpeg")
