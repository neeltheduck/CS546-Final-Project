...
if (toolregisterform) {
    console.log('toolregisterform');
     toolregisterform.addEventListener('submit', (event) => {
         ...
         else if(toolName.length < 2 || toolName.length > 40){
             error.innerHTML = 'Tool Name must be between 2 and 40 characters';
             error.hidden = false;
             return;
         }
         else if (description.length > 500 || description.length < 2){
             error.innerHTML = 'Description must be between 2 and 500 characters';
             error.hidden = false;
             return;
         }
         ...
         else if (location.length > 50 || location.length < 2){
             error.innerHTML = 'Location must be between 2 and 50 characters';
             error.hidden = false;
             return;
         }
         else{
 ...
 if(userregisterform){
     ...
     userregisterform.addEventListener('submit', (event) => {
         ...
         if(!username || !password || !confirmPassword || !pronouns || !bio || !userLocation || !themePreference){
             ...
             error.innerHTML = 'Username must be between 5 and 10 characters';
             error.hidden = false;
             return;
         }
             ...
             error.innerHTML = 'Confirm Password must at least 8 characters';
             error.hidden = false;
             return;
         }
         else if (pronouns.toLowerCase() != 'he/him' && pronouns.toLowerCase() != 'she/her' && pronouns.toLowerCase() != 'they/them'){
             error.innerHTML = 'Pronouns must be either He/Him, She/Her, or They/Them';
             error.hidden = false;
             return;
         }
         else if (bio.length > 255 || bio.length < 20){
             error.innerHTML = 'Bio must be between 20 and 255 characters';
             error.hidden = false;
             return;
         }
         else if (userLocation.length > 100 || userLocation.length < 2){
             error.innerHTML = 'Location must be between 2 and 100 characters';
             error.hidden = false;
             return;
         }
         else if (themePreference.toLowerCase() != 'dark' && themePreference.toLowerCaes() != 'light'){
             error.innerHTML = 'Theme Preference must be either Light or Dark';
             error.hidden = false;
             return;
         }
         else{
 ...
 if(ratingform){
     ...
     ratingform.addEventListener('submit', (event) => {
         ...
         if(!userID || !toolID || !rating || !comment){
             ...
             error.innerHTML = 'Comment must be at least 10 characters';
             error.hidden = false;
             return;
         }
         else{
